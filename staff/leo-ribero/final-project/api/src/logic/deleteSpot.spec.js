...
describe('deleteSpot', () => {
    ...
    describe('when user already exists', () => {
        ...
        describe('when user already has spots', () => {
            ...
            it('succeeds on correct user data', () =>
                deleteSpot(user.id, spot2.id)
                    .then(result => {
                        expect(result).to.be.undefined

                        return Spot.findById(spot2.id)
                    })
                    .then(spot2 => expect(spot2).to.be.null)
            )
        })
        ...
        describe('when user has no spots', () => {
            it('succeeds on correct user data', () => {
                ...
                return deleteSpot(user.id, unexistingSpotId)
                    .then(() => {
        ...
        it('fails on incorrect user id', () => {
            ...
            deleteSpot(wrongUserId, unexistingSpotId)
                .then(() => {
    ...
    describe('when user does not exist', () => {
        it('fails on unexisting user id', () => {
            ...
            deleteSpot(unexistingUserId, unexistingSpotId)
                .then(() => {
                    throw new Error('should not reach this point')
                })
                .catch(error => {
