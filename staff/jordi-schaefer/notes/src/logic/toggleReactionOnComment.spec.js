...
const { User, Note, Comment, Reaction } = require('../models')
const { NotFoundError } = require('../errors')
const toggleReactionOnComment = require('./toggleReactionOnComment')
const { expect } = require('chai')


describe('toggleReactionOnComment', () => {
        ...
        describe('When note already exists', () => {
            ...
            describe('When comment already exists', () => {
                ...
                it('succeeds on corret data', () => {
                    return toggleReactionOnComment(user2.id, note.id, comment.id, Reaction.LOVE)
                        .then(result => {
                            expect(result).to.be.undefined

                            return Note.findById(note.id)
                            .then(note => {
                                const _comment= note.comments.find(_comment => _comment._id.toString() === comment.id)
                                const reaction= _comment.reactions.find(react => react.user.toString() === user2.id)
