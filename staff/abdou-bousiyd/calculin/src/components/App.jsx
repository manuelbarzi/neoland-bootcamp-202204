...
class App extends Component {debugger
    ...
    handleButtonClick = event => {
        ...
        let incorrectCalc = signs.includes(key.substr(key.length - 1));

        if(key === '=') {
            if(incorrectCalc) return
        }
        ...
        if (key === 'AC') 
            return this.setState({ history: this.state.history = '', display: this.state.display = 0})
    ...
    render() {
        return <div className="App">
            <div className="App__display">
                <span id='operation'>{this.state.history}</span> <br />
                <span id='result'>{this.state.display}</span>
                {/* <span id='result'>{this.state.display}</span> */}
            </div>
