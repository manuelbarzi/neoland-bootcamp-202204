function validateJWT(token) {
    ...
    if (token.split('.').length !== 3 || !hasPartsFull) throw new Error('invalid token')
}
...
function validatePassword(password, explain) {
    ...
    if(password.length < 8) throw new Error(`${explain} has less than 8 characters`)
}

function validateNoteId(noteId) {
    validateString(noteId, 'note Id')
    const parts = noteId.split(".")
    if(!parts instanceof Array || parts.length !==2 || parts[0].length !== 13 || parts[1].length !== 10) throw new Error('invalid note Id')
}