...
class Apium {
     ...
     */
    call(method, urlOrPath, options, callback) {  // metodo,directorio,header y o body, espera de la respuesta que despues manejare
        const xhr = new XMLHttpRequest

        xhr.addEventListener('load', event => { // a la espera de la escucha de la respuesta 
            const { status, responseText: payload } = event.target // recogere los parametros 

            callback(null, { status, payload }) // donde manejo por estados 200 - 500
        })  // recogo parametros ( callback de call)
