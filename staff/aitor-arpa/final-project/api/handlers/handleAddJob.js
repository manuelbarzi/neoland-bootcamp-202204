...
module.exports = (req, res) => { // tengo un express pediente de si recive un POST a users, y me devuelve (req, res)
    try {
        ...
        const { body: { title,description,address,workers } } = req // extraigo las propiedades del body que me ha ggenerado jsonBodyParser


        addJob(userId,title,description,address,workers) // llamo a mi funcion
            .then(data => {
