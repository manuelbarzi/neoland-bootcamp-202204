...
describe('Add Work', () => {    
    ...
    beforeEach(() => {
        return Promise.all([User.deleteMany(), Job.deleteMany()])
            .then(() => {
                worker = new User({ name: 'Pringi', username: 'Currante', password: '123123123', role: 'worker', DNI: '1df2323123s', email: 'as@as.com' })
    ...
    it('succeeds on correct user', () => {
        return addJob(useradmin.id, 'TotKilain', 'Reparacion de Puerta', 'C/ escopeta Nº Pum')
            .then(newwork => {
    ...
    it('fail on Worker user add new work', () => {
        return addJob(worker.id, 'TotKilain', 'Reparacion de Puerta', 'C/ escopeta Nº Pum')
            .then(newwork => {
