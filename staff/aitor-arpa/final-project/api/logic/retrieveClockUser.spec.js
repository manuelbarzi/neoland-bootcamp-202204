...
describe('retrieveClockUse', () => {
    ...
    beforeEach(() => {
        return Promise.all([User.deleteMany(), Clock.deleteMany(),Job.deleteMany()])
            .then(() => {                              
                usercloc = new User({ name: 'Miguel', username: 'Mingu', password: '123123123', role: 'worker', DNI: '123123123s', email:'sadf@sdfs.vo' })
                return usercloc.save()
            })
            .then(user => {
                data = new Date
                newcloc = new Clock({user: user.id, job: null,timeout: data})
                newjob = new Job({titel:'sdfs',description: 'sdfasdf', address: 'njb',workers:[user.id] }) 
    ...
    it('sucees on user exist', () => {
        debugger
        return retrieveClockUser(usercloc.id)
    ...
    it('sucees on user not exist', () => {
        ...
        return retrieveClockUser(newjob.id)
