...
const { validateString, validateStringNotEmptyOrBlank, validateId } = require('validator')
...
function updateUser(userId, name, username, password, nid, email, role) {
    ...
    return User.findById(userId)
        .then(user => {
            ...
            return User.updateOne({ _id: userId }, { $set: { name, password, email, nid, username, role } })

        })
        .then(result=> {
            if (result.matchedCount === 0) 
            throw new ConflictError(`fail in update`)
        })
