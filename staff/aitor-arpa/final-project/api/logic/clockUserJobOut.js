  ...
  return Promise.all([
    ...
    if (!clock)
      throw new NotFoundError(
        `clock for user with id ${userId} and job with id ${jobId} does not exist`
      );

    if (!job)
      throw new NotFoundError(
        `job for id ${jobId} for user with id ${userId} does exist`
      );

    if (clock.user.toString() !== userId)
      throw new ConflictError(
        `clock with id ${clockId} does not correspond to user with id ${userId}`
      );

    if (clock.job.toString() !== jobId)
      throw new ConflictError(
        `clock with id ${clockId} does not correspond to job with id ${jobId}`
      );
