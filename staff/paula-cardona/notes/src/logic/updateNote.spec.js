        ...
        describe('when user already has notes', () => {
            ...
            it('user exists and note too', () => { //putisimas llaves return gracias
                ...
                return updateNote(userId, noteId, text) // llamo a nuestra funcion. con el return hago que no siga hasta que no retorne toda la cadena de .then
                    .then((result) => { //no quiero que la funcion me devuelva nada, solo quiero que actualize y yasta
                        expect(result).to.be.undefined

                        //return Note.findById(noteId)
                        return Note.findOne({ _id: noteId, user: userId }) //la vuelvo a traer para ver si se ha actualizado

                    })
                    .then((note) => { //miro que tiene dentro la nota para ver si se ha actualizado
                        expect(note.text).to.equal('bye') //expect(note.text).to.equal(text)
                    })
