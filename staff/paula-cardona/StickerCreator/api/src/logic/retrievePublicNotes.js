...
function retrievePublicNotes(userId) { // recibimos el user id al que les buscaremos sus notas 
    ...
    return User.findById(userId) //por tema de seguridad tenemos que primero mirar si existe el usuario 
        .then((user) => {
            ...
            return Note.find({ user: userId, audience: 1 }).lean()    //lean() me trae el documento    
        })
        .then((notes)=>{ 
            notes.forEach(note=>{
