describe('deleteNote', () => {
    it('succeeds when user and note already exist', () => {
        db.users.length = 0
        db.notes.length = 0   //limpiamos base de datos

        db.users.push(new User('Le Chunga', 'lechunga', '123123123'))
        db.users.push(new User('Papa Gayo', 'papagayo', '123123123'))  //creamos dos users

        let note   //creamos note

        db.notes.push(new Note('lechunga', 'Hello, Note! (1)'))    //creamos notas
    ...
    it('fails when user is not the owner of the note', () => {
        ...
        deleteNote('lechunga', note.id, error => {
            expect(error).toBeDefined()
